...
const renderer=new THREE.WebGLRenderer({canvas:$<HTMLCanvasElement>('world'),antialias:true,powerPreference:'high-performance'});renderer.setSize(innerWidth,innerHeight);renderer.setPixelRatio(Math.min(devicePixelRatio,1.5));renderer.shadowMap.enabled=true;renderer.shadowMap.type=THREE.PCFSoftShadowMap;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.05;
...
let landscape=new Landscape(settings.seed,settings.style);let drive=new Driving(landscape);drive.transmission=settings.transmission;const world=new WorldView(scene,landscape);world.season=settings.season;world.update(drive.z,drive.x);
...
renderer.domElement.addEventListener('pointerdown',e=>{if(e.button!==0||!started||paused||panel)return;mouseDragging=true;mousePointer=e.pointerId;mouseX=e.clientX;mouseY=e.clientY;mouseReleaseAt=performance.now()+2600;renderer.domElement.setPointerCapture(e.pointerId);});
renderer.domElement.addEventListener('pointermove',e=>{if(!mouseDragging||e.pointerId!==mousePointer)return;const dx=e.clientX-mouseX,dy=e.clientY-mouseY;mouseX=e.clientX;mouseY=e.clientY;mouseYaw=THREE.MathUtils.clamp(mouseYaw+dx*.0052,-1.9,1.9);mousePitch=THREE.MathUtils.clamp(mousePitch-dy*.0030,-.26,.34);mouseReleaseAt=performance.now()+2600;});
...
function atmosphere(){
 ...
 sky.material.uniforms.top.value.set(top);sky.material.uniforms.bottom.value.set(bottom);sky.material.uniforms.night.value=night?1:0;
 (scene.fog as THREE.Fog).color.set(bottom);(scene.fog as THREE.Fog).near=night?65:170;(scene.fog as THREE.Fog).far=night?420:560;scene.background=new THREE.Color(bottom);
 ...
 scene.environmentIntensity=night?.035:.45;renderer.toneMappingExposure=night?1.2:1.05;headlights=night;vehicle.setLights(headlights);
 $('place').textContent=`THE HIGHLANDS · ${night?'NIGHT':settings.season.toUpperCase()}`;save();
}
function quality(){renderer.setPixelRatio(settings.quality==='low'?.75:settings.quality==='medium'?1:Math.min(devicePixelRatio,1.5));renderer.shadowMap.enabled=settings.quality!=='low';sun.shadow.mapSize.set(settings.quality==='high'?2048:1024,settings.quality==='high'?2048:1024);sun.shadow.map?.dispose();sun.shadow.map=null;save();}
...
function frame(now:number){requestAnimationFrame(frame);const raw=(now-last)/1000,dt=Math.min(raw,.1);last=now;const controls=input();if(qaThrottle){controls.throttle=1;controls.brake=0;controls.steer=0;}
 ...
 document.body.dataset.drivingState=JSON.stringify({started,paused,auto:drive.auto,speed:drive.speed,x:drive.x,z:drive.z,y:drive.y,lateral:landscape.lateral(drive.x,drive.z),distance:drive.distance,view,gear:drive.gear,rpm:drive.rpm,transmission:drive.transmission,headlights,terrainTiles:world.fields.size,cameraDistance:camera.position.distanceTo(vehicle.group.position),cameraSeatError:view==='interior'?camera.position.distanceTo(desired):null,season:settings.season,slip:drive.slip,roll:drive.roll,pitch:drive.pitch,handbrake:controls.handbrake,...telemetry});prevYaw=yaw;
}
