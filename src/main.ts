...
const defaults={seed:'OPEN-ROAD',style:'flowing',season:'summer',light:'day',weather:'clear' as WeatherMode,quality:navigator.maxTouchPoints>0?'low':'high',resolution:'auto' as RenderResolution,renderBackend:'webgl',color:'#e5e8e2',volume:.35,muted:false,view:'chase',transmission:'automatic'};
...
renderer.setPixelRatio(renderPixelRatio(innerHeight,devicePixelRatio,graphicsQuality(settings.quality),settings.resolution));renderer.setSize(innerWidth,innerHeight);renderer.shadowMap.enabled=true;if(renderer instanceof THREE.WebGLRenderer)renderer.shadowMap.type=THREE.PCFSoftShadowMap;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.05;
