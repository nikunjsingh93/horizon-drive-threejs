...
export class WorldView {
 ...
 dashes(start:number){const w=this.landscape,pos:number[]=[],indices:number[]=[];for(let z=start;z<start+CHUNK;z+=12){const end=Math.min(z+4,start+CHUNK);for(const zz of [z,end])for(const off of [-.055,.055])pos.push(w.roadX(zz)+off*w.stretch(zz),w.roadY(zz)+.043,zz);const base=pos.length/3-4;indices.push(base,base+2,base+1,base+1,base+2,base+3);}const geo=new THREE.BufferGeometry();geo.setAttribute('position',new THREE.Float32BufferAttribute(pos,3));geo.setIndex(indices);geo.computeVertexNormals();const m=new THREE.Mesh(geo,this.lineMat);m.receiveShadow=true;return m;}
